...
feature "user visits their show page" do
  let!(:user_1) { User.create(email: 'jscho13@gmail.com',
                              ...
                              gender: 'M',
                              password: 'Applied91') }
  let!(:user_2) { User.create(email: 'krdixon83@gmail.com',
                              first_name: 'Dickson',
                              last_name: 'Lau',
                              gender: 'M',
                              password: 'Applied91') }
  let!(:user_3) { User.create(email: 'shinshiny@gmail.com',
                              first_name: 'Hannah',
                              last_name: 'Shin',
                              gender: 'F',
                              password: 'Applied91') }
  let!(:user_4) { User.create(email: 'jrpespisa@verizon.net',
                              first_name: 'John',
                              last_name: 'Pespisa',
                              gender: 'M',
                              password: 'Applied91') }
  let!(:user_5) { User.create(email: 'amywerner@gmail.com',
                              first_name: 'Amy',
                              last_name: 'Werner',
                              gender: 'F',
                              password: 'Applied91') }
  let!(:user_6) { User.create(email: 'leonrusso@gmail.com',
                              first_name: 'Leon',
                              last_name: 'Russo',
                              gender: 'M',
                              password: 'Applied91') }
  let!(:friend_1) { Friend.create(user_id: user_1.id,
                                  users_friend_id: user_2.id) }
  let!(:friend_2) { Friend.create(user_id: user_1.id,
                                  users_friend_id: user_3.id) }
  let!(:match) { Match.create(user_id: user_2.id,
                                  users_match_id: user_4.id) }
  let!(:match_2) { Match.create(user_id: user_2.id,
